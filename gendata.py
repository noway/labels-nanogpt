...
def generate_section_of_a_chapter_workbook(level, chapter, section):
    question = f'You\'re writing a math workbook for {level} level K-12. Write the "{section}" section of the "{chapter}" chapter. Make sure the section is thorough and complete. Include answers as if the student has already completed the excercises.'
    answer = ask_gpt4_chat(question)
    return answer
...
def generate_and_save_section_of_a_chapter_workbook(level, chapter, section):
...
def generate_practicebooks():
    ...
    with open(f'{level_codename}_toc.json') as f:
        ...
        for chapter in data:
            for section in data[chapter]:
                generate_and_save_section_of_a_chapter_practicebook(
